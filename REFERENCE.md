...
- Login / account is a **new surface**. It is not on the live site. Keep the game board and header looking like Parle (help | title | settings). Accedi lives in the settings sheet; when signed in, Account opens `/account` (Esci, Notifications, and account deletion live there), not a fourth header icon. Auth UI is WorkOS AuthKit hosted (email magic link + Google), in Italian.
